...
This project explores React class components, lifecycle methods, and performance optimization techniques. You'll learn when to use class vs functional components, how to implement lifecycle hooks, create Higher-Order Components (HOCs), and optimize component rendering for better performance.
...
- With `StrictMode`, you'll notice an extra `mount` and `unmount` cycles which is expected, [read more here](https://react.dev/reference/react/StrictMode#fixing-bugs-found-by-re-running-effects-in-development)
